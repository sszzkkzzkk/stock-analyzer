...
6:00  データ収集 + テーマ予測
9:05  寄り付き確認 + 予測差分を即学習 → 当日16:00に反映
16:00 大引け総括 + 最終学習 → 翌日6:00に反映
"""
...
2. 6:00予測との差（何が当たって何が外れたか）を分析する
3. 外れた理由を具体的に特定する（見落としたファクターは何か）
4. この差分を今日の16:00分析に活かすための修正ヒントを生成する
...
{{
  ...
  "intraday_correction": {{
    ...
    "correction_hints": ["16:00分析への修正ヒント1", "ヒント2", "ヒント3"]
  }},
...
【分析指示】
- 朝の予測から16:00までの流れを総括する
- 9:05の修正ヒントが活きたかを評価する
- 翌日の予測精度向上のための学習ポイントを生成する
