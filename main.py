...
client = anthropic.Anthropic(api_key=os.environ.get(“ANTHROPIC_API_KEY”, “dummy”))
...
for key in [
    "indices", "world_indices", "forex", "futures", "sox", "oil",
    "top_gainers", "top_losers", "volume_surge", "themes", "news"
]:
    print(f"{key}: {len(result[key])}")
# index.html の「指標ライブON」表示用に market_live.json を保存
live_items = []
...
for k in [“market_regime”, “market_regime_label”, “entry_style”,
“entry_style_label”, “danger_level”, “conclusion”]:
...
today_str = today.strftime(”%Y年%m月%d日”)
try:
prompt = build_analysis_prompt_1535(today_str, pred_600, pred_905, pred_1200, market, market[“news”] + nhk + reuters)
raw = call_claude(prompt, max_tokens=1600)
...
logs.append({
“date”: today.isoformat(),
“usefulness”: parsed[“review”].get(“next_focus”, “”),
“easy_pattern”: parsed[“review”].get(“actually_best_theme”, “”),
“danger_pattern”: parsed[“review”].get(“danger_theme_in_morning”, “”),
“next_note”: parsed[“review”].get(“next_ban_rule”, “”),
})
save(“review_log.json”, logs)
except Exception as e:
